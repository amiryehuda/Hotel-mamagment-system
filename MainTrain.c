...
void printHostel(Hostel* h)
{
	int size = strlen(h->hostel_name);
	for (int i = 0; i < size; i++)
	{
		printf("%c", h->hostel_name[i]);
	}
	printf("\n");
	printf("%d\n", h->num_of_rooms);
	printf("%f\n", h->rate);
...
int main()
{
	...
	//FreeRoom(p1);
	//Hostel* temphostel = &h3;
	//AddRoom(temphostel, &room1);
	//printf("a\n\n");
	//if (0 == checkRoom(temphostel, &room1))
	//	printf("Failed Ex3 - AddRoom function (-8)\n");
